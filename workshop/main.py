...
def part_one(env, vertexes):
    with open("machines_part_one.json", "w") as out_f:
        json.dump(obj=[_.to_dict() for _ in generate_good_graphs(env=env, vertexes=vertexes, vertex_count=7)], fp=out_f, sort_keys=True, indent=4)
    ...
    with open("machines_part_one.json") as json_file:
        ...
        for on_model_part in list(reversed(env.get_all_on_model())):
            for ms in machines:
                ...
                plot_multi(to_plot, filename="pics/" + str(m_id) + ":::" + str(on_model_part) + ":::" + str(ms.binary_matrix_representation) + ":::" + str(
        ...
        with open("machines_part_two.json", "w") as out_f:
            json.dump(obj=[_.to_dict() for _ in machines_to_save], fp=out_f, sort_keys=True, indent=4)


def part_three(env, vertexes):
    pass
