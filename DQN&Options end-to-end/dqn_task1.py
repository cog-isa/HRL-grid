...
def learn(env,
    ...
    pred_q = q_func(conv, num_actions, scope="task1", reuse=False)
    ...
    target_q = q_func(target_q_conv, num_actions, scope="task1_target_q_func", reuse=False)
    ...
    with tf.variable_scope("Hold_the_var"):
        ...
        q_func_vars1 = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="task1")
        target_q_func_vars0 = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='convolution_target_q_func')
        target_q_func_vars1 = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='task1_target_q_func')
    ...
    update_target_fn = []
    for var, var_target in zip(sorted(q_func_vars0, key=lambda v: v.name),
                               sorted(target_q_func_vars0, key=lambda v: v.name)):
        update_target_fn.append(var_target.assign(var))
    for var, var_target in zip(sorted(q_func_vars1, key=lambda v: v.name),
                               sorted(target_q_func_vars1, key=lambda v: v.name)):
        update_target_fn.append(var_target.assign(var))
