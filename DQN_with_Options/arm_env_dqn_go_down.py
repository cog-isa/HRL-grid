...
class ArmEnvDQN_1(CoreEnv):
    ...
    def __init__(self, size_x, size_y, cubes_cnt, scaling_coeff, episode_max_length, finish_reward, action_minus_reward,
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(size_x * self._scaling_coeff, size_y * self._scaling_coeff, 3))
...
#             self._grid[x, y] = 1
    ...
    def step(self, a, options=False):
        ...
        if self._arm_x+1 < self._size_x:
            if self._grid[self._arm_x+1, self._arm_y] == 1 and self._arm_x+2 >= self._size_x:
                self._done = True
            ...
            if self._grid[self._arm_x+1, self._arm_y] == 1 and self._arm_x+2 < self._size_x:
                if self._grid[self._arm_x+2, self._arm_y] == 1:
                    self._done = True
                    reward += self._finish_reward
                    self._episode_reward += self._finish_reward
                    info = True
    ...
    # @staticmethod
    # def render_to_image(self, n_grid):
    #     n_grid[self._arm_x, self._arm_y] = 3 - self._magnet_toggle * 1
    #     # n_grid = up_scaler(n_grid, 1)
    #
