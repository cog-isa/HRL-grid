...
def learn(env,
    ...
    pred_q = q_func(obs_t_float, num_actions, scope="q_func_1", reuse=False)
    ...
    with tf.variable_scope("Hold_the_var"):
        ...
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func_1')
    ...
    saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func_1'))
    ...
    meta_graph_def = tf.train.export_meta_graph(filename='dqn_graph2/dqn_graph.ckpt.meta', export_scope = 'q_func_1')
    save_path = saver.save(session, "dqn_graph2/dqn_graph.ckpt", write_meta_graph = False)
