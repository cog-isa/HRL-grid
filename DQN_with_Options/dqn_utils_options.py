...
def sample_n_unique(sampling_f, n):
    ...
    comparable objects (will be indexes) , sample n such unique objects.
    """
    res = []
    while len(res) < n:
...
def compute_exponential_averages(variables, decay):  # экспоненциальное скользящее среднее
...
class ReplayBuffer(object):
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.opt_steps = np.empty([self.size], dtype=np.float32)
