...
def learn(env,
    ...
    with tf.variable_scope('over_options'):
#         with tf.variable_scope("obs_t_ph"):
    ...
    with tf.variable_scope("Hold_the_var"):
        ...
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='over_options/q_func')
    ...
    saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='over_options'))
    ...
    for t in itertools.count():
        ...
        if action < env.action_space.n:
            ...
            next_obs, reward, done, opt_steps_n, info = options[action-env.action_space.n].step(env)
        ...
        if (t > learning_starts and t % learning_freq == 0 and 
            ...
            obs_batch, act_batch, rew_batch, next_obs_batch, done_batch, opt_steps_batch = replay_buffer.sample(batch_size)
    ...
    meta_graph_def = tf.train.export_meta_graph(filename='dqn_over_options_6_4/dqn_graph.ckpt.meta', export_scope = 'over_options')
    save_path = saver.save(session, "dqn_over_options_6_4/dqn_graph.ckpt", write_meta_graph = False)
