...
class ArmEnvOpt2(ArmEnv):
    ...
    def _step(self, a):
        ...
        if self.get_tower_height() > self.tower_height and self._magnet_toggle == False:
            self._done = True
...
def q_learning_opt(env, num_episodes, q_table, initial_states, term_states, eps=0.2, alpha=0.4, gamma=1.0):
...
def main():
    ...
    for i in range(n):
        env = ArmEnvOpt2(episode_max_length=200,
                 size_x=7,
                 size_y=7,
                 cubes_cnt=5,
                 ...
                 finish_reward=200,
                 tower_target_size=5)
        stats, q_table, initial_states, term_states = q_learning_opt(env, 3000, q_table, initial_states, term_states)
    ...
    #
    # with open('opt_q_table_v2.txt', 'wb') as handle:
    #     pickle.dump(q_table, handle)
    #
    # with open('opt_init_st_v2.txt', 'wb') as handle:
    #     pickle.dump(initial_states, handle)
    #
    # with open('opt_term_st_v2.txt', 'wb') as handle:
